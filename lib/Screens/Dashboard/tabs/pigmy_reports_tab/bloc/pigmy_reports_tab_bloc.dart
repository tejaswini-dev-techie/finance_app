...
part 'pigmy_reports_tab_event.dart';
part 'pigmy_reports_tab_state.dart';

class PigmyReportsTabBloc
    extends Bloc<PigmyReportsTabEvent, PigmyReportsTabState> {
  PigmyReportsTabBloc() : super(PigmyReportsTabLoading()) {
    on<GetPigmyReports>((event, emit) async {
  ...
  Future<void> _mapGetPigmyReportsDetailsEventToState(
      GetPigmyReports event, Emitter<PigmyReportsTabState> emit) async {
    try {
      ...
      if (isInternetConnected) {
        ...
        (reportData != null)
            ? emit(PigmyReportsTabLoaded())
            : emit(PigmyReportsTabError());
      } else {
        //Internet state
        emit(PigmyReportsTabNoInternet());
      }
    } catch (e) {
      emit(PigmyReportsTabError());
    }
